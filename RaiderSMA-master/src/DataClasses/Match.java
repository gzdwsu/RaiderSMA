...
public class Match implements Comparable, Serializable{
    ...
    //I added these two methods to help with advance
    public void setRedPoints(int rp) {
    	this.redPoints = rp;
    }
    public void setGreenPoints(int gp) {
    	this.greenPoints = gp;
    }
    ...
    //this is a member function that I added myself to help with advancing the tournament;
    public String getWinningColor() {
    	return this.winningColor;
    }

    //this function needs to be examined as it may be responsible for some of the errors that occur when using advance
    public String getWinner() { //Returns only a username so that the wrestler's record can be updated in the model.
        if (complete) { //note: complete always seems to be false.
    ...
    public void updateMatch(int fallType, String winningColor, int greenPoints, int redPoints, String fallTime) {
        ...
        complete = true; //undid the comment //commented this out, want to see if that will have an effect on information changing
    }
