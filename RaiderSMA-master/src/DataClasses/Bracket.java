...
public class Bracket implements Serializable {
    ...
    public void initializeBracket() {
        ...
        for (int i = 0, j = wrestlerList.size() - 1; i < j; i++, j--) { //int i = 0, j = wrestlerList.size() - 1; i < j; i++, j-- <-ORIGINAL
            temp.add(new Match(0, wrestlerList.get(i).getUserName(), wrestlerList.get(i).getTeamID(), wrestlerList.get(j).getUserName(),wrestlerList.get(j).getTeamID(), currentRound, weightClass));//the last two parameters that use i were originally j
        }
